...
var data = {
    ...
    tempId: utils.getQueryString('tempId'),
    createTemplateUrl: utils.getQueryString('createTemplateUrl'),
...
var methods = {
    ...
    //},
    getDisplayUrl: function (channelId, contentId) {
        return 'http://www.zhijiao361.com/template/contents/' + channelId + '/' + contentId + '.html';
    },
    ...
    getCreateType: function () {
        if (this.createType == 'cloud') {
            ...
            return '使用在线站点模板创建站点，站点模板：<a href="' + this.getDisplayUrl(this.channelId, this.tempId) + '" target="_blank">' + this.createTemplateId + '</a>';
    ...
    btnCloudClick: function () {
        ...
        //this.load();
        window.location.href = "siteTemplateOnline.cshtml";
    },
        ...
        this.$validator.validate().then(function (result) {
            if (result) {
                ...
                $api.post($url, {
                    ...
                    channelId: $this.channelId,//栏目id
                    contentId: $this.tempId,//内容id
                    createTemplateUrl: encodeURIComponent($this.createTemplateUrl) //模板文件地址
                }).then(function (response) {
                    var res = response.data;
